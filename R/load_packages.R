...
load_packages = function(pkgs = NULL, auto_install = F, update = F,
  ...
  if (sum(!result) > 0) {
    cat("\n\nThese packages need to be installed:",
        paste(pkgs[!result], collapse = ", "), "\n")
    install_code = paste0('install.packages(c("',
                          paste(pkgs[!result], collapse = '", "'), '"))')
    cat(install_code, "\n")
    ...
    if (auto_install) {
      ...
      capture.output({
        result_retry = sapply(pkgs[!result], require, character.only = T,
                              quietly = T)
      })
    }
  } else {
  ...
  results = list(packages = pkgs,
                 pkgs_result = result,
                 pkgs_retry = result_retry,
